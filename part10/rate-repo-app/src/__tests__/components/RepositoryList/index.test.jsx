...
describe('RepositoryList', () => {
  describe('RepositoryListContainer', () => {
    test.only('renders repository information correctly', () => {
      const repositories = {
        totalCount: 8,
        pageInfo: {
          hasNextPage: true,
      ...
      const values = repositories.edges.map(({ node }) => {
        const { id, ownerAvatarUrl, ...props } = node
        return props
      })

      const itemHasTextContents = (item, textContents) =>
        Object.values(textContents).forEach((value) => {
          let text
          ...
          const element = within(item).getByText(text)
          expect(element).toHaveTextContent(text)
        })

      itemHasTextContents(firstItem, values[0])
      itemHasTextContents(secondItem, values[1])
    })
  })
})
